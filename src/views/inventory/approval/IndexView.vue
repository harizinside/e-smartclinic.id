    ...
    <VTable
      ...
      <VTableColumn
        ...
        <td class="px-6 py-4 text-xs">
          <span v-if="row.approval.value" :class="[row.approval.value ? 'bg-blue-700' : 'bg-red-700', 'p-2 rounded-md text-white']">{{ $d(Date.parse(row.approval.date), 'long') }}</span>
          <span v-else>-</span>
        </td>
        <td class="px-6 py-4">
...
import JsonData from '@/utils/po-approval.json'
...
const navs = ref<INavigation[]>([
  ...
  { name: 'Approval', link: '/inventory/approval', active: true }
])
